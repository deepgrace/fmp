...
namespace fmp
{
    ...
    template <typename T>
    extern const wrapper_t<T> wrapper;
    ...
    template <typename T>
    requires std::is_aggregate_v<T>
    consteval decltype(auto) unpack()
    {
        auto [...args] = T();

        return forward_as_fuple(args...);
    }

    template <typename T>
    using unpack_t = decltype(unpack<T>());

    template <typename T>
    requires std::is_aggregate_v<T>
    consteval decltype(auto) member_addresses()
    {
        auto& [...args] = wrapper<T>.value;

        return make_fuple(std::addressof(args)...);
    }

    template <typename T>
    using member_addresses_t = decltype(member_addresses<T>());

    template <size_t N, typename T>
    using member_address_t = fuple_element_t<N, member_addresses_t<T>>;

    template <size_t N, typename T>
    inline constexpr auto member_address_v = get<N>(member_addresses<T>());
    ...
    template <size_t N, typename T>
    consteval decltype(auto) member_name()
    {
        return member_stem<member_address_v<N, T>>();
    }

    template <size_t N, typename T>
    inline constexpr auto member_name_v = member_name<N, T>();

    template <typename T, typename U>
    consteval decltype(auto) member_index()
    {
        return fuple_index<T, 1>(unpack<U>());
    }
    ...
    consteval decltype(auto) member_name()
    {
        return member_stem<member_address_v<member_index_v<T, U>, U>>();
    }

    template <typename T>
    requires std::is_aggregate_v<T>
    consteval decltype(auto) member_names()
    {
        return [&]<size_t... N>(std::index_sequence<N...>)
        {
            return make_fuple(member_name_v<N, T>...);
        }
        (std::make_index_sequence<arity_v<T>>());
    }

    template <typename T>
    inline constexpr auto member_names_v = member_names<T>();
    ...
    template <typename T>
    constexpr decltype(auto) member_index(const std::string_view& s)
    {
        return apply([&]<typename... Args>(Args&&... args)
        {
            return search_index(s, std::forward<Args>(args)...);
        }, member_names_v<T>);
    }

    template <size_t N, typename T, auto = 0>
    using type_t = fuple_element_t<N, unpack_t<T>>;

    template <size_t N, typename T>
    struct member
    {
        using type = type_t<N, T>;
    };
    ...
    {
        return fmp::for_each(std::forward<F>(f), tie_fuple(std::forward<T>(t)));
    }
    ...
    {
        return fmp::apply(std::forward<F>(f), tie_fuple(std::forward<T>(t)));
    }
    ...
    constexpr decltype(auto) choose(T&& t)
    {
        if constexpr(is_tuple_v<std::remove_cvref_t<T>>)
            ...
            return fmp::tie(fmp::get<N>(std::forward<T>(t))...);
    }
    ...
    using fuple_ref_t = ruple<1, fmp::fuple, Args...>;
    ...
    {
        return fmp::for_each(std::forward<F>(f), member_pointers_v<std::remove_cvref_t<T>>);
    }
    ...
    constexpr static S& apply(S& s, T&& t)
    {
        ...
        if constexpr(B)
            s << fmp::quoted(fmp::get<lower>(std::forward<T>(t)));
        else
            s >> fmp::quoted(fmp::get<lower>(std::forward<T>(t)));
    ...
    struct less<void>
    {
        ...
        {
            return fmp::lt(std::forward<T>(t), std::forward<U>(u));
        }
    ...
    struct not_equal_to<void>
    {
        ...
        constexpr bool operator()(T&& t, U&& u) const
        {
            return fmp::ne(std::forward<T>(t), std::forward<U>(u));
        }
    ...
    struct less_equal<void>
    {
        ...
        constexpr bool operator()(T&& t, U&& u) const
        {
            return fmp::le(std::forward<T>(t), std::forward<U>(u));
        }
    ...
    struct equal_to<void>
    {
        ...
        constexpr bool operator()(T&& t, U&& u) const
        {
            return fmp::eq(std::forward<T>(t), std::forward<U>(u));
        }
    ...
    struct greater_equal<void>
    {
        ...
        constexpr bool operator()(T&& t, U&& u) const
        {
            return fmp::ge(std::forward<T>(t), std::forward<U>(u));
        }
    ...
    struct greater<void>
    {
        ...
        constexpr bool operator()(T&& t, U&& u) const
        {
            return fmp::gt(std::forward<T>(t), std::forward<U>(u));
        }
