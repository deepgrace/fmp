...
namespace fmp
{
    ...
    constexpr decltype(auto) for_each(F&& f, T&& t)
    {
        return [&]<size_t... N>(std::index_sequence<N...>)
        {
            if constexpr(is_fuple_v<std::remove_cvref_t<T>>)
                (..., std::invoke(std::forward<F>(f), fmp::get<N>(std::forward<T>(t))));
            else
    ...
    static constexpr decltype(auto) expand(U&& u)
    {
        if constexpr(f)
        {
            return fmp::apply([]<typename... Args>(Args&&... args)
            {
                if constexpr(t)
                    return fmp::tie(std::forward<Args>(args)...);
                else
        ...
        else
        {
            return std::apply([]<typename... Args>(Args&&... args)
            {
                if constexpr(t)
                    ...
                    return fmp::make_fuple(std::forward<Args>(args)...);
            }, std::forward<U>(u));
        }
    }
